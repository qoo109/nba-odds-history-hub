...
Do not include cookies, authorization headers, account information, or HAR files. The quality report will show which `matchupId` values matched instead of silently assuming the two files are complete.
...
The quality report includes matched and unmatched `matchupId`, duplicate IDs, orphan participants, invalid prices, SHA-256, and insert counts.

## 6. Example using synthetic samples
...
`observed_at` is not the same as `cutoff_at`, `scheduled_tipoff`, or `ingested_at`. A second `observed_at` is required before the project can describe movement between two points.
